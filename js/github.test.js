...
export async function runGitHubTests() {
        ...
        if (url.includes('config.json')) {
            return {
                ...
                json: async () => ({
                    ...
                    defaultAddresses: { bootloader: "0x1000", partitions: "0x8000", app: "0x10000" }
                })
            };
        }
        if (url.includes('api.github.com')) {
            return {
                ...
                json: async () => ({
                    tag_name: "v1.0.0",
                    assets: [
                        { name: "firmware.bin", browser_download_url: "http://example.com/fw.bin" },
                        { name: "bootloader.bin", browser_download_url: "http://example.com/bl.bin" }
                    ]
                })
            };
        }
    ...
    try {
        ...
        console.assert(manifest.builds[0].parts.length === 2, "Should have 2 parts");
        
        const fakeButton = document.createElement('esp-web-install-button');
        document.body.appendChild(fakeButton);
        const url = applyManifestToButton(manifest);
        console.assert(fakeButton.getAttribute('manifest') === url, "Manifest URL should be set");
