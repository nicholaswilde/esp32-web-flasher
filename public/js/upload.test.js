...
export async function runUploadTests() {
  ...
  try {
    const file = new File(["dummy content"], "app.bin", {
      type: "application/octet-stream",
    });
    const manifest = await createManifestFromFiles([
      { file: file, address: "0x10000" },
    ]);

    console.assert(
      manifest.name === "Local Manual Upload",
      "Manifest name should be correct",
    );
    console.assert(manifest.builds[0].parts.length === 1, "Should have 1 part");
    console.assert(
      manifest.builds[0].parts[0].offset === 65536,
      "Offset should be 0x10000 (65536)",
    );
