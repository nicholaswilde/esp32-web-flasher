...
export async function fetchReleaseManifest(repo, release, device) {
  ...
  let targetAssets = release.assets.filter((a) =>
    a.name.match(/\.(bin|zip)$/i),
  );
  if (device) {
    targetAssets = targetAssets.filter((a) =>
      a.name.toLowerCase().includes(device.toLowerCase()),
    );
  }

  if (targetAssets.length === 0) {
    throw new Error(
      `No firmware assets found for device ${device} in this release`,
    );
  }
  ...
  if (zipAsset) {
    ...
    if (!zipBuffer) {
      throw new Error(
        "Failed to download zip file: all CORS proxies failed or were rate-limited.",
      );
    }
...
export function applyManifestToButton(manifest) {
  ...
  const manifestUrl =
    "data:application/json;charset=utf-8," + encodeURIComponent(manifestStr);
